...
def find_particular_solution(a: int, b: int, c: int) -> Tuple[int, int]:
    ...
    if c % d != 0:
        raise ValueError(
            f"No integer solutions exist: gcd({a}, {b}) = {d} does not divide {c}")
...
def create_plot(a: int, b: int, c: int, solutions: List[Tuple[int, int, int]]):
    ...
    max_coord = max(max(abs(x) for x in x_coords), max(abs(y)
                    for y in y_coords))
    ...
    fig.update_layout(
        ...
        showlegend=True,
        annotations=[
            dict(x=window_size*0.9, y=window_size*0.9,
                 text="Quadrant I (+,+)", showarrow=False),
            dict(x=-window_size*0.9, y=window_size*0.9,
                 text="Quadrant II (-,+)", showarrow=False),
            dict(x=-window_size*0.9, y=-window_size*0.9,
                 text="Quadrant III (-,-)", showarrow=False),
            dict(x=window_size*0.9, y=-window_size*0.9,
                 text="Quadrant IV (+,-)", showarrow=False)
        ]
    )
...
def main():
    try:
        ...
        with left_col:
            ...
            a = st.slider("Select a", -350, 350, 308, 1)
            b = st.slider("Select b", -350, 350, 259, 1)
            c = st.number_input("Enter c", value=21, step=1)
            max_solutions = st.slider(
                "Maximum solutions to display", 5, 50, 20)
        ...
        with right_col:
            ...
            st.write(
                f"- Particular solution closest to origin: x₀ = {x0}, y₀ = {y0}")
